--[[*****************************************************************************
**** Note       : Breakpoints Repaired 2022/08/22
****
...
function trace(event,line)
	-- Pause According to Active Break Lines
	if(dbcmd.bps[line] ~= nil) then
		for k, v in pairs(dbcmd.bps[line]) do
			if(v.active) then
				dbcmd.trace = true
			end
		end
	...
	if dbcmd.trace then
		...
		print(line.."   |"..src)
		while not execute_cmd(env, line) do end
	end
